...
myKeys conf@XConfig { XMonad.modMask = modm } =
  ...
  -- ThinkPad docking
       , ((modm .|. shiftMask, xK_plus), spawn "autorandr dock")
       , ((modm .|. shiftMask, xK_minus), spawn "~/Desktop/single.sh")
       ]
    ++
...
myMouseBindings XConfig { XMonad.modMask = modm } = M.fromList
    ...
    -- mod-shift-button1, Set the window to floating mode and move by dragging
  [ ( (modm, button3)
...
--
myLayoutHook = spacingRaw False (Border 2 5 5 5) True (Border 5 5 5 5) True
...
myManageHook = composeAll
  ...
  , className =? "Spotify"    --> doShift "9:dvi"
  ]
...
myStartupHook :: X ()
myStartupHook = do
  spawn "$HOME/.xmonad/scripts/autostart.sh" -- startup script
  setWMName "LG3D"
